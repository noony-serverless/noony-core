...
Provides Zod integration patterns for Noony: schema definition with `z.infer`, the parsedBody-to-validatedBody pipeline, middleware ordering per `middleware-ordering`, Pub/Sub message validation, and async validation with database lookups.
...
- You need middleware ordering beyond validation -> `middleware-ordering` is the canonical reference
- You need custom middleware development -> `middleware-development`
- You need path parameter validation -> `path-parameters` handles route params
...
2. **Verify middleware ordering per `middleware-ordering`**: ErrorHandlerMiddleware (position 1) -> BodyParserMiddleware (position 6) -> BodyValidationMiddleware (position 7) — parser converts raw body to `parsedBody`, validator checks `parsedBody` against schema and populates `validatedBody`
...
- `BodyParserMiddleware` MUST come before `BodyValidationMiddleware` — positions 6 and 7 in the canonical order (see `middleware-ordering`)
- `ErrorHandlerMiddleware` MUST be present at position 1 (see `middleware-ordering`) — without it, `ValidationError` crashes the function instead of returning a clean 400
...
- Forgetting `BodyParserMiddleware` before `BodyValidationMiddleware` — positions 6 then 7 per `middleware-ordering`; reversing them breaks the pipeline
...
- You know middleware ordering per `middleware-ordering`: ErrorHandler (1) -> BodyParser (6) -> BodyValidation (7)
