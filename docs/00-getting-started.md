...
> If you implement a custom middleware, it must carry the same generics as the handler — otherwise the type chain breaks silently. See [01-handler-guide.md](./01-handler-guide.md) for why this matters.
...
- Understand why the middleware chain and lifecycle work the way they do: [01-handler-guide.md](./01-handler-guide.md)
- Understand how global and per-request dependencies are managed without cloning: [03-container-architecture.md](./03-container-architecture.md)
- Full API signatures and parameter tables: [02-api-reference.md](./02-api-reference.md)
