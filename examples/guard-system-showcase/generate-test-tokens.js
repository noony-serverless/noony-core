...
function generateToken(user, testRunId = null) {
  ...
  const email = testRunId
    ? user.email.replace('@', `+${testRunId}@`)
    : user.email;
...
if (require.main === module) {
  ...
  if (args.includes('--help') || args.includes('-h')) {
    ...
    console.log('Usage:');
    console.log(
      '  node generate-test-tokens.js                        # Generate all tokens'
    );
    console.log(
      '  node generate-test-tokens.js --user basic           # Generate specific user token'
    );
    console.log(
      '  node generate-test-tokens.js --test-run-id test123  # Generate with test run ID'
    );
    console.log(
      '  node generate-test-tokens.js --verify TOKEN         # Verify a token'
    );
    console.log('');
    console.log('Options:');
    console.log('  --user TYPE        Generate token for specific user type');
    console.log(
      '  --test-run-id ID   Generate unique tokens for isolated testing'
    );
